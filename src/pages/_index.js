import Login from './auth/login'
import MainIndex from './main/index'
import Products from './main/products-pages/products'
import NotFound from './partials/not-found'
import DashboardPage from './main/dashboard'
import ExpensesPage from './main/expenses'
import PaymentPage from './main/payment'
import CashLedger from './main/cash'
import TransactionPage from './main/transaction'
import AddProduct from './main/products-pages/add-product'
import ProductIndex from './main/products-pages/product-index'
import CategoryIndex from './main/product-categories-pages/category-index'
import Categories from './main/product-categories-pages/categories'
import AddProductCategory from './main/product-categories-pages/add-product-category'
import StoreIndex from './main/store-pages/store-index'
import AddStore from './main/store-pages/add-store'
import Stores from './main/store-pages/stores'
import CrewIndex from './main/crew-pages/crew-index'
import CrewPage from './main/crew-pages/crew'
import AddCrew from './main/crew-pages/add-crew'
import UpdateStore from './main/store-pages/update-store'
import UpdateCrew from './main/crew-pages/update-crew'
import UpdateProductCategory from './main/product-categories-pages/update-product-categories'
import UpdateProduct from './main/products-pages/update-product'

export {
    Login,
    MainIndex,
    Products,
    NotFound,
    DashboardPage,
    ExpensesPage,
    PaymentPage,
    CashLedger,
    TransactionPage,
    AddProduct,
    ProductIndex,
    CategoryIndex,
    Categories,
    AddProductCategory,
    StoreIndex,
    AddStore,
    Stores,
    CrewIndex,
    CrewPage,
    AddCrew,
    UpdateStore,
    UpdateCrew,
    UpdateProductCategory,
    UpdateProduct
}